...
def export_csv(data: Union[List[Dict[str, Any]], Dict[str, Any]],
    ...
    """
    Export either:
      - trade log (list[dict])  -> logs/trade_log_{strategy_name}_{ts}.csv
      - summary report (dict)   -> reports/summary_{strategy_name}_{ts}.csv

    ts = timestamp ברמת דקה, למשל: 20251127_2034
    """

    # חותמת זמן ברמת דקה (בלי שניות)
    ts = datetime.now().strftime("%Y%m%d_%H%M")


    # ---- case 1: trade log (list of dict rows) ----
    if isinstance(data, list) and data and isinstance(data[0], dict):
        return _export_trade_log_csv(data, strategy_name, ts)

    # ---- case 2: summary report (dict with sections) ----
    if isinstance(data, dict) and any(k in data for k in ("meta", "performance", "trades_summary")):
        return _export_summary_report_csv(data, strategy_name, ts)
...
def _export_trade_log_csv(trade_table: List[Dict[str, Any]],
    ...
    """
    Internal: export trade log (list of dict) to logs/trade_log_{strategy}_{ts}.csv
    """
...
def _export_summary_report_csv(report: Dict[str, Any],
    ...
    as a single-row CSV:
        reports/summary_{strategy_name}_{ts}.csv
    """
    out_path = Path(out_dir)
    out_path.mkdir(parents=True, exist_ok=True)

    filename = f"summary_{strategy_name}_{ts}.csv"
